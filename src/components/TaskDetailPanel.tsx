...
export function TaskDetailPanel({ taskId, onClose }: TaskDetailPanelProps) {
  const { board, updateTask, deleteTask, addChecklistItem, updateChecklistItem, deleteChecklistItem } = useBoard();

  // Find the task
  const task = board?.tasks.find((t) => t.id === taskId);

  // Find category name
  const category = task ? board?.categories.find((c) => c.id === task.categoryId) : null;
  ...
  const statusOptions: { value: TaskStatus; label: string; activeClass: string }[] = [
    { value: 'not_started', label: STATUS_LABELS.not_started, activeClass: `${STATUS_COLORS.not_started.bg} ${STATUS_COLORS.not_started.border}` },
    { value: 'in_progress', label: STATUS_LABELS.in_progress, activeClass: `${STATUS_COLORS.in_progress.bg} ${STATUS_COLORS.in_progress.border}` },
    { value: 'completed', label: STATUS_LABELS.completed, activeClass: `${STATUS_COLORS.completed.bg} ${STATUS_COLORS.completed.border}` },
  ];
  ...
  const assigneeOptions: { value: TaskAssignee; label: string; activeClass: string }[] = [
    ...
    { value: 'bride', label: ASSIGNEE_LABELS.bride, activeClass: `${ASSIGNEE_COLORS.bride} border-[#AF7A76]` },
    { value: 'groom', label: ASSIGNEE_LABELS.groom, activeClass: `${ASSIGNEE_COLORS.groom} border-[#637569]` },
    { value: 'both', label: ASSIGNEE_LABELS.both, activeClass: `${ASSIGNEE_COLORS.both} border-[#29564F]` },
    { value: 'other', label: ASSIGNEE_LABELS.other, activeClass: `${ASSIGNEE_COLORS.other} border-gray-400` },
  ];
  ...
  const priorityOptions: { value: TaskPriority; label: string; activeClass: string }[] = [
    ...
    { value: 'high', label: PRIORITY_LABELS.high, activeClass: 'bg-[#9A3F3C]/15 text-[#9A3F3C] border-[#9A3F3C]' },
  ];
  ...
  return (
    <div className="fixed top-0 right-0 h-full w-96 max-w-[calc(100vw-48px)] border-l bg-white flex flex-col shadow-lg z-50">
      ...
      <div className="flex justify-between items-center p-4 border-b bg-gray-50">
        <span className="text-sm text-gray-500">{category?.name || 'Unknown Category'}</span>
        <button
          onClick={onClose}
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-5">
        ...
        {/* Notes - LARGE textarea (prominent!) */}
        <div>
          <label className="block text-xs font-medium text-gray-500 mb-1">Notes</label>
          <textarea
            value={task.notes}
            onChange={(e) => updateTask(task.id, { notes: e.target.value })}
            className="w-full h-48 border border-gray-300 rounded-lg p-3 text-sm resize-none focus:border-[#29564F] focus:ring-1 focus:ring-[#29564F] focus:outline-none"
        ...
        <div className="pt-2 border-t">
          ...
          <button
            ...
            className="flex items-center gap-2 text-sm text-gray-600 hover:bg-gray-50 p-1 -m-1 rounded w-full text-left transition-colors"
          >
            {isChecklistExpanded ? (
              ...
              {task.checklist.map((item) => (
                ...
                >
                  <input
                    ...
                    onChange={() => updateChecklistItem(task.id, item.id, { completed: !item.completed })}
      ...
      <div className="p-4 border-t bg-gray-50">
        <button
          ...
          className="w-full py-2 text-sm text-[#861930] border border-[#861930]/30 rounded hover:bg-[#861930]/10 transition-colors"
        >
          Delete Task
        </button>
      </div>
